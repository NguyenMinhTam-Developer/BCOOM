...
class AddressFormPage extends GetView<AddressFormController> {
  ...
  static void navigateToEdit(int addressId) {
    Get.toNamed(
      AppPageNames.addressEdit.replaceAll(':id', addressId.toString()),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          controller.isEditMode ? 'Chỉnh sửa địa chỉ' : 'Thêm địa chỉ mới',
          style: AppTextStyles.heading5.copyWith(color: AppColors.text500),
        ),
        elevation: 0,
      ),
      body: Obx(() {
        // Show loading indicator while loading address data in edit mode
        if (controller.isLoading.value && controller.isEditMode && controller.editingAddress == null) {
          return const Center(child: CircularProgressIndicator());
        }

        // Get initial values - use loaded address data if available, otherwise use defaults
        final initialValues = <String, dynamic>{
          'address_type': 'home', // Default to home
        };

        if (controller.isEditMode && controller.editingAddress != null) {
          final address = controller.editingAddress!;
          initialValues.addAll({
            ...
            'province_code': controller.selectedProvinceCode.value ?? address.province?.provinceCode,
            'ward_code': controller.selectedWardCode.value ?? address.ward?.wardCode,
          });
        }
                        ...
                        return Obx(
                          () => InputLabel(
                            ...
                            child: FormBuilderDropdown<String>(
                              name: 'province_code',
                              initialValue: controller.selectedProvinceCode.value ?? (controller.isEditMode && controller.editingAddress != null ? controller.editingAddress!.province?.provinceCode : null),
                              decoration: InputDecoration(
                                ...
                                () => InputLabel(
                                  ...
                                  child: controller.isLoadingWards.value
                                      ...
                                      : FormBuilderDropdown<String>(
                                          name: 'ward_code',
                                          initialValue: controller.selectedWardCode.value ?? (controller.isEditMode && controller.editingAddress != null ? controller.editingAddress!.ward?.wardCode : null),
                                          decoration: InputDecoration(
              ...
              Expanded(
                ...
                child: Obx(
                  () => ElevatedButton(
                    ...
                    child: Text(
                      controller.isLoading.value ? 'Đang xử lý...' : (controller.isEditMode ? 'Lưu thay đổi' : 'Lưu địa chỉ mới'),
